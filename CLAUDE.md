...
**This frontend filter is a UI convenience, not the security boundary — real isolation is enforced by RLS (migration 53, added 2026-08-08).** Every employee has `employees.branch_id` (their assigned home branch) and `employees.all_branches_access` (boolean, default `false`). Today only Monica Torres has `all_branches_access = true`; Kevin Mijangos is scoped to Puerto Vallarta and Carlos/Daniel/Diego/Gigi/Mar are scoped to Puebla. `private.can_read_branch(target_branch_id)`/`private.can_write_branch(target_branch_id)` (private schema, security definer) implement the check: read allows a null `branch_id` (shared/unresolved row) or a match with the caller's own branch or `all_branches_access`; write requires an exact match (or `all_branches_access`) — a row can never be created/edited as null or as another branch by a scoped employee.

**Implementation is a single `as restrictive` policy per table**, not a rewrite of the ~40 existing permissive role-based policies — Postgres ANDs restrictive policies against whatever permissive policy already granted access, so this is purely additive (existing role policies still decide *whether* a role can touch a table at all; the restrictive policy narrows *which rows*). Applied to: `customers`, `products`, `supply_purchases`, `service_tickets`, `transactions`, `inventory_movements`, `invoices`, `pos_sales`, `pos_returns`, `discount_codes`, `service_prices`, `team_tasks`, `brand_assets`, `employees` (with a `auth_user_id = auth.uid()` self-bypass so a user can always read/update their own row), and — deriving branch from the parent row since these have no `branch_id` of their own — `customer_devices` (via `customers.branch_id`), `attachments` (via `service_tickets.branch_id` or `customers.branch_id`; falls through to unrestricted for support-task attachments, which aren't branch-scoped), `ticket_events`/`ticket_items` (via `service_tickets.branch_id`), `pos_sale_items` (via `pos_sales.branch_id`), `pos_return_items` (via `pos_returns.branch_id`). All scoped `to authenticated` only, so the anon-role policies used by public pages (`ticket-track.html`, `ticket-tech.html`) are untouched.

**`notifications` only gets the restrictive policy on `SELECT`, not `INSERT`/`UPDATE`** — deliberately. Most notifications are targeted (`recipient_id` set) with `branch_id` left `null` by `addNotif()`'s default; a write-side restriction would have blocked routine ticket/task notifications for any employee without `all_branches_access`. A broadcast (`branch_id` explicitly set) is now correctly contained to that branch on the read side; a broadcast left `branch_id: null` still reaches everyone, unchanged from before.

**`suppliers`, `service_types`, `app_settings`, `support_tasks`, `support_task_comments`, `audit_log` are intentionally NOT branch-restricted** — global catalogs/config or IT-support tooling that isn't branch-scoped by design (Monica does IT for both branches).

**`create-employee` Edge Function enforces the same boundary independently** — it runs on the service-role key, which bypasses RLS entirely, so without its own check a branch-scoped admin (e.g. Kevin) could create/edit/reset/delete an employee in the *other* branch through the Edge Function even though RLS blocks the equivalent direct-table write. `assertBranchAllowed()` in `supabase/functions/create-employee/index.ts` rejects `create`/`update`/`delete`/`reset_password` when the target employee's (or new employee's) `branch_id` doesn't match the caller's, unless the caller has `all_branches_access`.

**`applyDiscount()`/`activeDiscountCodesForScope()` now go through `branchDiscounts()`** (filters `state.discounts` by `!d.branchId || d.branchId === <active branch id>`) instead of scanning `state.discounts` unfiltered — before this fix a discount code created for one branch could be typed in and applied at the other, since the code lookup itself had no branch awareness (independent of the `discount_codes` RLS row-visibility fix in migration 53).

**Frontend branch switcher (`.branch-tabs` in the topbar) is hidden for anyone without `all_branches_access`** (`updateBranchTabsVisibility()`, called right after `resolveCurrentEmployee()` on login) and `setActiveBranch()` no-ops for them — `activeBranchId` is locked to `currentEmployee.default_branch_id` for the whole session. This is UX, not the security boundary (RLS holds regardless of what `activeBranchId` says client-side) — it just avoids the confusing "I clicked Puebla and everything's empty" experience for someone who was never going to see Puebla data anyway. The "Sucursal" field on the employee create/edit form (`openForm("employee")`) is similarly locked to the creator's own branch when they lack `all_branches_access` (visually locked via `pointer-events:none` + a revert-on-change listener, not `disabled` — a disabled `<select>` doesn't submit its value in `FormData`, which would have sent `branch_id: null`).

**Historical data backfill**: at the time of migration 53, all real data (57 customers, 71 tickets, 147 products, etc.) already had `branch_id` pointing at Puerto Vallarta — the only branch that had been operating. The 2 stray `transactions` rows with `branch_id is null` were backfilled to Puerto Vallarta too. Puebla starts with zero data, which is correct — it's a new branch about to go live.
...
- **Segmentación por sucursal requires migration 53** (`53_branch_segmentation.sql`): without it, `employees.all_branches_access`/the per-employee `branch_id` assignment don't exist and every RLS `"branch isolation"` restrictive policy is missing — the app falls back to the old fully-permissive behavior (any active employee reads/writes both branches' data through the API regardless of what the frontend shows). See "Multi-branch data isolation" above for the full design. **When granting a new hire `all_branches_access`, do it by hand in SQL** (`update employees set all_branches_access = true where id = '...'`) — there is no UI for it yet, and it should stay a rare, deliberate grant (today: only Monica).
...
- **Login se quedaba trabado en "Verificando..." tras contraseña correcta, solo un refresh completo lo destrababa (fixed 2026-08-08)**: dos bugs distintos, ambos en el mismo flujo. (1) `handleLogin()` cancela su watchdog de 20s (`loginTimeout`) apenas `attemptSupabaseLogin()` resuelve, *antes* de llamar a `afterLogin()` — así que si `resolveCurrentEmployee()` o `reloadState()` se colgaban a medio camino por un problema de red real, no había ningún timeout cubriendo esa segunda etapa. Se agregó `withTimeout()` (`Promise.race` contra un `setTimeout`) envolviendo ambas llamadas dentro de `afterLogin()` (15s / 25s), para que un cuelgue de red dispare el `catch` existente (cierra sesión, muestra error) en vez de congelar la UI indefinidamente. (2) **La causa real del caso reportado no era de red, era de DOM/CSS**: `showChangePasswordScreen()` nunca ocultaba `#login-screen` (a diferencia de `showApp()`, que sí lo hace). Como ni `#login-screen` ni `#change-password-screen` usan `position:fixed` — son `<div>` normales en flujo de documento, cada uno `min-height:100vh` (`app.css`) — cuando un login exitoso disparaba `force_password_change`, ambas pantallas quedaban montadas simultáneamente, apiladas verticalmente: el login viejo (con el botón "Verificando..." nunca reseteado, porque `afterLogin()` sí había terminado bien y no tenía motivo para tocar ese botón) llenaba el primer viewport completo, y la pantalla real de cambio de contraseña quedaba renderizada — funcional, pero invisible — más abajo. Un refresh "arreglaba" todo porque recargaba desde cero sin la pantalla vieja montada. Fix: `showChangePasswordScreen()` ahora oculta `#login-screen` explícitamente, igual que `showApp()`. **Lección de debugging**: una prueba automatizada headless que solo verifica que el DOM final tenga el estado correcto (`#change-password-screen` visible) puede pasar sin detectar este bug, porque ese elemento *sí* existe y *sí* tiene `display:flex` — el problema es puramente de qué tapa a qué visualmente. Para bugs reportados como "se ve congelado/trabado", inspeccionar el `display`/stacking de *todas* las pantallas candidatas al mismo tiempo, no solo si la pantalla esperada se activó.
