...
- **"¿Cómo nos conocieron?" requires migration 34**: `34_customer_referral_source.sql` adds `customers.how_found`/`how_found_other`. The field is in the cliente form (`formSchemas.client`, options from `REFERRAL_SOURCES`) and saved by both `createRemoteClient`/`updateRemoteClient`. `how_found_other` is only persisted when `how_found === "Otro"`. `#reports-referral` groups `branchClients()` by `howFound` for clients whose `createdAt` (mapped from `customers.created_at`) falls in the selected period — clients created before migration 34 show under "Sin especificar".
