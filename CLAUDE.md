...
- **Centro de notificaciones requires migration 31**: `31_notifications.sql` creates the `notifications` table. Without it, `state.notifications` stays `[]`, the badge never shows unread counts, and `addNotif()` (sending an "aviso al equipo") fails with a table-not-found error. `addNotif()` is `async` — callers must `await` it. Only `admin`/`it`/`owner` can INSERT (send broadcasts); any active employee can SELECT and mark as read. Automatic triggers (ticket assigned, stage change, IT reply) and the IT support comment thread are **not** implemented yet — this migration only covers the manual "aviso al equipo" broadcast feature (Fase 1).
