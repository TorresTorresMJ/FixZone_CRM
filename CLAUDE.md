...
- **Afluencia de clientes (day×hour heatmap of Recibido/Entregado visits)**: rendered in two places — the full card at `#reports-traffic` (`renderTrafficHeatmap()`, respects the Reportes period filter, has the Ambos/Recibido/Entregado toggle) and the compact `#dashboard-traffic-chart` widget (`renderDashboardTraffic()`, always all-time history, no toggle). Both pull from `ticket_events` (not `delivered_at`, which only stores a bare date with no time-of-day) via `fetchTrafficEvents()`/`loadTrafficBucket()`, sharing one `trafficCacheMap` keyed by `` `${activeBranchId}|${from}|${to}` `` so the two widgets' different date ranges don't evict each other's cache. **The `event_type` filter must include `"quote_approved"`, not just `"created"`/`"stage_change"`**: `approveQuoteToTicket()` updates `stage` directly (bypassing `updateRemoteTicket()`) and logs its own event type for the cotización→ticket conversion, which is how most tickets in this business actually enter "Recibido" — omitting it silently starves the Recibido bucket while Entregado (always logged via `"stage_change"`) looks unaffected. **Pitfall (fixed by migration 49):** the ticket creation form (`formSchemas.ticket`) has no "status" field — the create-path code only ever set `data.status` explicitly in the cotización branch, so a regular walk-in ticket had `r.status === undefined` all the way into `createRemoteTicket()`. The INSERT still worked (`service_tickets.stage` has a DB-level default of `'Recibido'`, and Supabase's client drops `undefined` keys before sending JSON), so the ticket *looked* fine everywhere the app reads `ticket.status` — but `logTicketEvent(id,"created",{toStage:r.status})` recorded `to_stage: null` instead of `"Recibido"` (invisible to the heatmap), and the `recibido_sealed_at` seal never fired (gated on `r.status === "Recibido"`, never true). Fixed by explicitly setting `data.status = "Recibido"` in the non-cotización create branch, mirroring the cotización branch. **If Recibido counts look low again, check this exact class of bug first**: any code path that inserts/updates `service_tickets.stage` without going through `createRemoteTicket()`/`updateRemoteTicket()` (or that sets it via a DB column default instead of an explicit JS value) will look correct in the UI while silently breaking `ticket_events`/`recibido_sealed_at`, since both are driven off the JS value, not the DB's resolved column value.
